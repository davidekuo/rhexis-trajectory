...
def predict(args):
    ...
    with torch.no_grad():
        ...
        # print("sorted_images: ", images)
        images_ = images[:-gap]
        # print("images w/ gap: ", images_)
        ...
        for index, imfile1 in enumerate(images_):
            ...
            # print(type(flow_up), flow_up.shape, '\n', flow_up)

            flopath = os.path.join(floout, os.path.basename(svfile))
            # print("flopath: ", flopath)
            rawflopath = os.path.join(rawfloout, os.path.basename(svfile))
            # print("rawflopath: ", rawflopath)

            flo = flow_up[0].permute(1, 2, 0).cpu().numpy()
            print("flo: ", type(flo), flo.shape, "\n", flo)

            # save raw flow
            # print("raw flow filename: ", rawflopath[:-4]+'.flo')
            writeFlowFile(rawflopath[:-4] + ".flo", flo)

            # save image.
            flo = flow_viz.flow_to_image(flo)
            # print("flow_vis: ", type(flo), flo.shape, '\n', flo)
            # print("flow_vis filename: ", flopath[:-4]+'.png')
