...
def generate_csv_dict(DATA_LOC, subdir, BEST_MODEL_CHECKPOINT, use_image_folder = True):
  ...
  yaml_filepath = os.sep + os.path.join(*str.split(BEST_MODEL_CHECKPOINT,os.sep)[0:-1],"config.yaml")
  print(yaml_filepath)
  cfg = None
  ...
  for frame in frame_list:
    ...
    if len(bbox) == 0:
      ...
      correct_keypoint_i = determine_correct_keypoints(keypoint_pairs, pupil_median, extents)
