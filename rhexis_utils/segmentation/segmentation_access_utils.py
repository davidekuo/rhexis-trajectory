...
def get_image_from_image_filename(
    filename_substring: str, DATA_LOC: str, subdir_names=[], use_image_dir=True
):
    """
    ...
    """
    return filename_to_numpy(
        get_full_image_filename(
            filename_substring, DATA_LOC, subdir_names, use_image_dir
        )
    )
...
def get_full_image_filename(
    ...
    if len(files_found) > 1:
        ...
        assert (
            False
        ), f"More than one image filename contains the substring '{filename_substring}'"

    # If no file was found, throw an error
    assert (
        len(files_found) == 1
    ), f"No image filename that contains the substring '{filename_substring}' was found"

    return files_found[0]


def get_label_from_image_filename(
    filename_substring: str, DATA_LOC: str, subdir_names=[], use_image_dir=True
):
    """
    ...
    file_found = get_full_image_filename(
        filename_substring, DATA_LOC, subdir_names, use_image_dir
    )
